...
import { ELM_BASE_URL_MAP, PAGES, PC_ENVIRONMENT_CLIENT_ID_MAP, PC_TO_ELM_ENVIRONMENT_MAP } from './enums.js'
...
async function authenticateGenesysCloud(appParams) {
  ...
  const clientID = PC_ENVIRONMENT_CLIENT_ID_MAP[pcEnvironment] || config.clientID;
...
async function install() {
  ...
  try {
    ...
    if (customSetupStatus.status) {
      ...
      //uninstall if ELM API provisioning is not successful
      await wizard.uninstall();
...
async function onCustomSetupEnter() {
  elmEnv = PC_TO_ELM_ENVIRONMENT_MAP[pcEnvironment];

  if (!elmEnv) {
    showErrorPage(
      getTranslatedText('txt-error-custom-setup'),
      getTranslatedText('txt-error-custom-setup-not-found-env-msg'),
      'txt-error-custom-setup',
      'txt-error-custom-setup-not-found-env-msg'
    );
  }
}

function afterCustomSetupPage() {
  let ccid;
  const el = document.getElementById('custom-ccid');
  ...
  if (!ccid) {
    alert(getTranslatedText('txt-error-custom-setup-empty-ccid-msg'));
